...
- If a provider file such as `memory/integrations/jira.md` or `memory/integrations/confluence.md` exists, align only its local folders, templates, ledgers, and draft locations.
- Do not require Atlassian MCP or any other external tool during initialization checks; missing tools or auth should be handled by the provider-specific skill at use time.
