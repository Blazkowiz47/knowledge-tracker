In this knowledge-base repository, perform the conflict-safe non-coordinator device sync workflow only.
...
This wrapper is for non-coordinator devices only. Use `KB_NODE_NAME` as the node name. Do not run `/sync` or `/aggregate-sync` from this prompt.
...
This is an unattended cron automation. Do not ask questions. Do not run day summarisation. Do not recommend focus items or next actions.
