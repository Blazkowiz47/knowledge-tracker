...
2. Confirm consolidated project-local memory skills exist for `remember`, `scratch`, `organise-scratch`, and `check-initialisation` under both `.agents/skills/` and `.claude/skills/`.
...
5. Treat old direct-name skills and `memory/commands/` as compatibility surfaces. Remove exact-template copies only after `remember` covers their behavior; preserve/report custom instructions.
